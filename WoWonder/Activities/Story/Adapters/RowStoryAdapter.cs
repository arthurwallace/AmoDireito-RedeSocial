...
namespace WoWonder.Activities.Story.Adapters
{
    public class RowStoryAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
            ...
            try
            {
                ...
                if (viewHolder is RowStoryAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        //holder.About.Text = Type == ContactsAdapter.TypeTextSecondary.About ? Methods.FunString.SubStringCutOf(WoWonderTools.GetAboutFinal(users), 25) : Methods.Time.TimeAgo(int.Parse(users.LastseenUnixTime));
        ...
        public override long GetItemId(int position)
        {
            try
            {
                return int.Parse(StoryList[position].UserId);
            }
            catch (Exception exception)
            {
                Console.WriteLine(exception);
    ...
    public class RowStoryAdapterViewHolder : RecyclerView.ViewHolder
    {
            ...
            Action<RowStoryAdapterClickEventArgs> longClickListener) : base(itemView)
        {
            try
            {
                ...
                Name.Gravity = GravityFlags.CenterVertical;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
