    ...
    public class MembersAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
        ...
        private void Initialize(MembersAdapterViewHolder holder, UserDataObject users)
        {
            try
            {
                ...
                holder.About.Text = users.UserId == UserDetails.UserId ? ActivityContext.GetString(Resource.String.Lbl_Online): ActivityContext.GetString(Resource.String.Lbl_Last_seen) + " " +Methods.Time.TimeAgo(int.Parse(users.LastseenUnixTime));
