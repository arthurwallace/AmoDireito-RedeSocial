                ...
                if (viewHolder is PokesAdapterViewHolder holder)
                {
                    ...
                    { 
                        if (item.UserData?.UserDataClass != null)
                        {
                            ...
                            holder.About.Text = ActivityContext.GetString(Resource.String.Lbl_Last_seen) + " " + Methods.Time.TimeAgo(int.Parse(item.UserData?.UserDataClass.LastseenUnixTime));

                            //Online Or offline
                            var online = WoWonderTools.GetStatusOnline(int.Parse(item.UserData?.UserDataClass.LastseenUnixTime), item.UserData?.UserDataClass.LastseenStatus);
