    ...
    public class WRecyclerView : RecyclerView, IOnLoadMoreListener
    {
        ...
        private void Init(Context context)
        {
            try
            {
                ...
                GetRecycledViewPool().SetMaxRecycledViews((int)PostModelType.AdMob, 10);
        ...
        private void MainScrollEvent_LoadMoreEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    item = list.LastOrDefault(a => a.TypeView != PostModelType.Divider && a.TypeView != PostModelType.ViewProgress && a.TypeView != PostModelType.AdMob && a.TypeView != PostModelType.FbAdNative && a.TypeView != PostModelType.AdsPost && a.TypeView != PostModelType.SuggestedGroupsBox && a.TypeView != PostModelType.SuggestedUsersBox && a.TypeView != PostModelType.CommentSection && a.TypeView != PostModelType.AddCommentSection);
                        ...
                        else
                        {
                            ...
                            foreach (var post in from post in result.Data let check = NativeFeedAdapter.ListDiffer.FirstOrDefault(a => a?.PostData?.Id == post.Id) where check == null select post)
                            {
                                ...
                                if (NativeFeedAdapter.NativePostType == NativeFeedType.Global)
                                {
                                    ...
                                    if (check1 == null && AppSettings.ShowSuggestedGroup && NativeFeedAdapter.ListDiffer.Count % AppSettings.ShowSuggestedGroupCount == 0 && ListUtils.SuggestedGroupList.Count > 0)
                                    {
                                    ...
                                    if (check2 == null && AppSettings.ShowSuggestedUser && NativeFeedAdapter.ListDiffer.Count % AppSettings.ShowSuggestedUserCount == 0 && ListUtils.SuggestedUserList.Count > 0)
                                    {
                                        combiner.AddSuggestedBoxPostView(PostModelType.SuggestedUsersBox);
                                    }

                                    if (NativeFeedAdapter.ListDiffer.Count % AppSettings.ShowAdMobNativeCount == 0 && AppSettings.ShowAdMobNativePost)
                                    {
                                        combiner.AddAdsPostView(PostModelType.AdMob);
                                    }
                                    else if (NativeFeedAdapter.ListDiffer.Count % AppSettings.ShowFbNativeAdsCount == 0 && AppSettings.ShowFbNativeAds)
                                    {
