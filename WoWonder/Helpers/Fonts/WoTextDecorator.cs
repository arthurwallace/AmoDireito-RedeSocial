    ...
    public class WoTextDecorator
    {
        ...
        public SpannableString SetupStrings(PostDataObject item, Context mainContext)
        {
            try
            {
                ...
                if (publisher.IsPro == "1")
                {
                    textIsPro = " " + "[img src=post_icon_flash/]";  
                    textHighLighter += textIsPro;
                }
                ...
                if (!string.IsNullOrEmpty(item.PostTraveling))
                {
                    textTraveling = "  " + "[img src=post_icon_airplane/]" + " " + mainContext.GetText(Resource.String.Lbl_IsTravelingTo) + " " + item.PostTraveling;  // IonIconsFonts.Plane
                    textHighLighter += textTraveling;
                }

                if (!string.IsNullOrEmpty(item.PostWatching))
                {
                    textWatching = "  " + "[img src=post_icon_eye/]" + " " + mainContext.GetText(Resource.String.Lbl_IsWatching) + " " + item.PostWatching; //IonIconsFonts.Eye
                    textHighLighter += textWatching;
                }
                 
                if (!string.IsNullOrEmpty(item.PostPlaying))
                {
                    textPlaying = "  " + "[img src=post_icon_game_controller_b/]" + " " + mainContext.GetText(Resource.String.Lbl_IsPlaying) + " " + item.PostPlaying; //IonIconsFonts.IosGameControllerB
                    textHighLighter += textPlaying;
                }

                if (!string.IsNullOrEmpty(item.PostListening))
                {
                    textListening = "  " + "[img src=post_icon_musical_notes/]" + " " + mainContext.GetText(Resource.String.Lbl_IsListeningTo) + " " + item.PostListening; //IonIconsFonts.Headphone
                    textHighLighter += textListening;
                }

                if (!string.IsNullOrEmpty(item.PostMap))
                {
                    textLocation = "  " + "[img src=post_icon_pin/]" + " " + item.PostMap.Replace("/", ""); //IonIconsFonts.Location
                    textHighLighter += textLocation;
                }
                ...
                //if (item.GroupRecipientExists != null && item.GroupRecipientExists.Value && !string.IsNullOrEmpty(item.GroupRecipient.GroupName))
                //{
                //    textUserRecipient += " " + "[img src=post_icon_arrow_forward/]" + " " + Methods.FunString.DecodeString(item.GroupRecipient.GroupName); //IonIconsFonts.ArrowRightC
                //    textHighLighter += textUserRecipient;
                //}
                //else if (item.RecipientExists != null && item.RecipientExists.Value && item.Recipient.RecipientClass != null)
                //{
                //    textUserRecipient += " " + "[img src=post_icon_arrow_forward/]" + " " + Methods.FunString.DecodeString(WoWonderTools.GetNameFinal(item.Recipient.RecipientClass)); //IonIconsFonts.ArrowRightC
                //    textHighLighter += textUserRecipient;
                //}
                ...
                DecoratedContent = TextViewWithImages.GetTextWithImages(item ,mainContext, new Java.Lang.String(textHighLighter.ToArray(), 0, textHighLighter.Count()));
